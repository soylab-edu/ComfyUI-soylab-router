...
`Runway` or `BytePlus` in a model name identifies its maker. `Comfy` in the Provider control is Comfy Router's default execution route, not the maker's direct API. The [official Router provider coverage](https://docs.comfy.org/development/comfy-router/providers) lists no `Dreamina` alternate execution route for Seedance 2.5 and no `Runway` alternate route for Gen-4 Turbo. Those names are not offered as selectable Router routes because the documented API cannot call them through `model_provider`. A Runway direct API price in the header is a separate comparison.
...
Until Router publishes route-specific pre-run prices, [`web/pricing.json`](web/pricing.json) tracks dated, sourced **direct provider API USD reference rates** for comparable settings. For alternate routes, the header converts that direct USD rate using Comfy's published **$1 = 211 credits** rate and labels the result as a reference estimate. It is not a Router quote or a guaranteed charge. The comparison panel shows the reference credits for the selected duration; after a run, an actual Router credit header takes precedence. To refresh the data, verify each provider's own price page, update the rate, `checked_at`, `updated_at` and source URL, review the panel with the matching model settings, then commit the JSON change. A provider's previous actual Router credit usage remains local to the browser; add a Git-tracked Router rate only after its exact settings and billed credits have been independently verified. If Comfy later publishes official route prices, use that source in preference to these direct API comparisons.
